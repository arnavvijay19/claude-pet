...
(`package-lock.json` and `node_modules/` are not committed — lockfile intentionally excluded per `.gitignore`'s `node_modules/`; add `package-lock.json` to the commit if reproducible installs matter later.)
