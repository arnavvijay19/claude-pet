...
const LEGACY_RENDERERS = new Set(['settingswindow.js', 'settings-preload.js', 'responsewindow.js', 'response-preload.js']);
...
function verifyPackage(packageRoot, { readFile = fs.readFileSync, walk = fs.readdirSync } = {}) {
  ...
  const visit = (directory) => { for (const entry of walk(directory, { withFileTypes: true })) { const full = path.join(directory, entry.name); const relative = path.relative(root, full); const stat = fs.lstatSync(full); if (stat.isSymbolicLink() || stat.isBlockDevice() || stat.isCharacterDevice()) fail(relative, 'reparse-object'); const lowerName = entry.name.toLowerCase(); const rule = FORBIDDEN.get(lowerName); if (rule) fail(relative, rule); if (LEGACY_RENDERERS.has(lowerName)) fail(relative, 'legacy-renderer'); if (/\.map$/i.test(entry.name)) fail(relative, 'source-map-suffix'); if (stat.isDirectory()) { visit(full); continue; } if (!stat.isFile()) fail(relative, 'unsupported-object'); files += 1; bytes += stat.size; if (stat.size <= 1024 * 1024 && /\.(?:js|json|txt|md|html|css)$/i.test(entry.name)) { const text = readFile(full, 'utf8').replaceAll('Bearer __OWNER_BEARER__', 'Bearer fixture'); if (/['"](?:response|settings):[a-z-]+/.test(text)) fail(relative, 'legacy-channel'); if (/-----BEGIN [A-Z ]*PRIVATE KEY-----|Bearer\s+[A-Za-z0-9._-]{16,}|sk-[A-Za-z0-9_-]{20,}|(?:ghp_|github_pat_|xoxb-|AKIA)[A-Za-z0-9_-]{12,}/i.test(text)) fail(relative, 'secret-pattern'); } } };
